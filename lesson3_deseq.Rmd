...
Let's look at a summary of our results. We will also set an **alpha** of 0.05 to tell DESeq which significance threshold to use when summarising results:
...
result_list<-coefficients %>% map(~lfcShrink(dds,coef = .x,type = "apeglm") %>% 
                                    ...
                                    rownames_to_column("geneID") %>% # move rownames to a column called geneID
...
We will also save these results tables to text files so we can use them outside of R if required. We will create a folder for each of our results.

```{r}
## Map each of our coefficients to a function that saves each table in our results_list
coefficients %>% map(function(x){
  dir.create(x)
  write_tsv(result_list[[x]],paste0(x,"/DEseq_result.tsv"),col_names = T,)
})
```
...
```{r}
ggplot(resLFC,aes(baseMean,log2FoldChange,)) + 
  ylim(-3,3) + scale_x_log10() + theme_bw() + ggtitle(paste("MA Plot:",c[1],c[2],"vs",c[3])) +
  geom_hline(yintercept = 0,colour="black") + theme(legend.key = element_blank())
```
