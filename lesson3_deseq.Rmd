...
result_list2<-result_list %>% 
  map(~as.data.frame(.x) %>% 
        ...
        mutate(threshold=case_when(padj<=padj_thresh & abs(log2FoldChange)>=l2fc_thresh~"Signifcant",T~"Not Significant")) %>% 
        arrange(padj) %>% 
        as_tibble()
  )
result_list2
...
result_list_anno<-result_list2 %>% 
  map(~left_join(x=.x,y=anno,by=c("geneID"="gene_id")) %>% 
...
### Heatmap of genes with largest changes

First, let's subset our results to get lists of significant genes
